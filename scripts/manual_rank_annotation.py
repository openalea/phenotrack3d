...
trackedplant.align_mature(start=0, gap=12, w_h=0.03, w_l=0.004, gap_extremity_factor=0.2, n_previous=5000)

# start = -1 better than 0 on one 1 plant
trackedplant.align_mature(start=-1, gap=1.8, w_h=0.03, w_l=0.004, gap_extremity_factor=0.8, n_previous=5000)
...
# TODO: seperate image/pl computations from rank computations: will be faster to switch between algo / auto
annot = {}
previous_ranks = None
for snapshot in trackedplant.snapshots:
# for snapshot in trackedplant.valid_snapshots():
    ...
    for angle in angles:
        ...
        annot[task]['images'][angle] = cv2.cvtColor(cache.image_client.imread(image_path), cv2.COLOR_BGR2RGB)
    ranks = snapshot.get_ranks()

    # # unvalid snapshots
    # if all([r == 0 for r in ranks]) and previous_ranks is not None:
    #     # use ranks of previous task if no predicted ranks (i.e. unvalid snapshot)
    #     # TODO: is it really usefull ? (slowing down the process, not visualised in 3D)
    #     ranks = previous_ranks[:len(ranks)]
...
for task, annot_task in annot.items():
    ...
    if len(ranks) != len(set(ranks)):
        raise Exception('task', snapshot.metainfo.task, ': several leaves have the same rank ! Cannot save')
    snapshot = next(s for s in trackedplant.snapshots if s.metainfo.task == task)
    for leaf_info in annot_task['leaves_info']:
        x_tip, y_tip, z_tip = [int(k) for k in snapshot.leaves[leaf_info['rank_phm'] + 1].real_pl[-1]]  # not super safe
